...
def main():
    ...
    #open_files(path, spec)
    #set_save_directory(path, spec)
    #splice_correction(spec)
    ascii_export(spec, path)

    


def open_files(path, spec):
...
def set_save_directory(path, spec):
...
def select_all(spec):
    for i in range(spec.ListBox.ItemCount()):
        spec.ListBox.select(i)
